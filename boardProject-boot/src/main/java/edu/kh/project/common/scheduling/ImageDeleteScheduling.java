...
@Slf4j
@Component // Bean 등록 
@PropertySource("calsspath:/config.properties")
public class ImageDeleteScheduling {
	...
	public void sheduling() {
		...
		// 두 배열을 하나로 합침 
		// imageArr 라는 빈 배열을 boardArr 과 memberArr 길이 만큼의 크기로 만든다 .
		File[] imageArr = new File[boardArr.length + memberArr.length];
		
		// 배열 내용 복사(깊은 복사)
		// System.arraycopy(복사할 배열, 몇번인덱스부터 복사할지, 
		//					새로운 배열, 새로운배열의 몇번인덱스부터넣을지,
		//					복사를 어디까지할건)
...
*
*
*  @Scheduled(cron = "30 * * * * *") // 시계 초 단위가 30인 경우 수
