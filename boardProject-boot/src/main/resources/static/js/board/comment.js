...
// -> 자원의 이름(주소)를 명시하고 
//    HTTP Metho(GET.POST,PUT,DELETE)를 
//    이용해 지정된 자원에 대한 CRUD 진행 

// 댓글 목록 조회 (ajax)
const slectCommentList = () => {
...
addComment.addEventListener("click", () => {
    ...
    // 댓글 내용이 작성되지 않은 경우
    if(commentContent.ariaValueMax.trim().length == 0){
...
const showInsertComment = (parentCommentNo, btn) => {
   ...
   */
   ...
   const insertChildComment = (parentCommentNo, btn) => {
    ...
    // ajax를 이용해 전달할 데이터 
    const data = {
        "commentContent" : commentContent.value, // 작성한 글
...
// 댓글 수정 fetch 요청 
cosnt updateComment = (commentNo, btn) => {
