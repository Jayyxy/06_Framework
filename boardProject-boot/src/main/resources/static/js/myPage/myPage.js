...
imageInput.addEventListener("change", () => {
  ...
  const file = imageInput.files[0]; // 실제로 작성하는 
  ...
  if(file){ // 파일이 선택된 경우
    ...
    if(file.size <= MAX_SIZE ) { // 현재 선택한 파일의 크기가 허용범위 이내인 경우 (정상인 경우)
      ...
      if(previousFile){
        ...
        dataTrasfer.items.add(previousFile); 
        imageInput.files = dataTrasfer.files; 
    ...
    if (previousFile){ // 이전 파일이 존재한다면 
      ...
      // DataTransfer.item.remove() : 파일 제거
      // DataTransfer.files : FileList 객체를 반환 
      // -> <input type="file"> 요소에 파일을 동적으로 설정.
      // --> input 태그의 files 속성은 File 객체를 FileList 변환하여 할당 진행 
      // DataTransfer를 이용하여 현재 File 객체를 Files
...
deleteImage.addEventListener("click", () => {
  ...
  if(profileImg.src !== defaultImageUrl) {
    ...
    previousFile = null ;  // 이전 파일 기록하는 변수에 null 로 초기화
  } else { 
    // 기본 이미지 상태에서 삭제 버튼 클릭 시 변경하지 않음
