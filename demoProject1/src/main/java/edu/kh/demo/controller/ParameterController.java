...
public class ParameterController {
	 ...
	 * - 주입되는 데이터는 매개변수의 타입에 맞게 형변환 자동으로 수행됨.
	 * 
	 * [기본 작성법]
	 ...
	 * -> "" (빈문자열)일 때는 에러 발생X
	 * (파라미터 존재하지 않는것이 아니라 name속성값=""로 넘어오기 때문에)
	 *  
	 * defaultValue : 파라미터 중 일치하는 name속성값이 없을 경우에 대입할 값 지정.
	 * -> required=false 인 경우 사용 
	 * */
	
	@PostMapping("test2") // /parmam/test2 POST 방식 요청 매핑
	...
	public String paramTest4(@ModelAttribute MemberDTO inputMember) {
		...
		log.debug("inputMem : " + inputMember);
		
		return "redirect:main"; // 상대경로 작성법 
		// == "redirect:/param/main"
		//		(현재위치가 중요!
		//		-> 현재 경로의 가장 마지막 부분을 redirect: 이주소로 변경하기 때문에)
